...
bien (`deployment_status`), `publicar-web.yml` se relanza sobre `main`
(`workflow_dispatch`), comprueba que el commit pertenezca a `main` y verifica y
publica la web de ese mismo commit. Si el API falla, la web anterior sigue publicada. La configuración del servicio de Railway —build,
