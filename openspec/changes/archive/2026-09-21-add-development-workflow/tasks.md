...
- [x] 3.3 Empujar `main` al remoto estableciendo el seguimiento, y verificar en la web de GitHub que el repositorio muestra el commit inicial con los 111 archivos y que `main` es la rama predeterminada
  > Verificada por la API de GitHub (`gh`), no mirando la web: rama predeterminada `main`, commit inicial `6cf507b`, 111 archivos idénticos a los locales. Por decisión del equipo se aceptó como equivalente. Después, al adoptar Gitflow (9.1), la rama predeterminada pasó a ser `develop`.
...
- [x] 6.4 **[manual]** Publicar la aplicación (la publicación ocurre al integrar `release-0.1.0` en `main`, tarea 9.6) y verificar abriendo `https://ragudc1984.github.io/idrconsultingcapacitacion/` desde otro dispositivo, sin el proyecto instalado, que la lista de tareas carga y es usable
  > Verificada por el equipo desde un celular, sin el proyecto instalado: la lista carga, se agrega una tarea y persiste tras recargar. Antes se comprobó con Playwright contra la URL real, en escritorio y en móvil emulado.
...
- [x] 9.6 Publicar la primera versión: crear `release-0.1.0` desde `develop`, fijar `"version": "0.1.0"` en `package.json`, integrarla por pull request en `main`, confirmar que `deploy` publica, crear la etiqueta `v0.1.0` sobre el commit de integración, e integrar `release-0.1.0` de vuelta en `develop`
  > Etiqueta anotada `v0.1.0` sobre `de0a011`; `release-0.1.0` integrada en `main` (#8) y de vuelta en `develop` (#9), y borrada después.
