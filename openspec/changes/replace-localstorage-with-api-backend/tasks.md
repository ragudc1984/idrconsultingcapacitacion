...
- [x] 9.2 **[manual]** Registrar el deploy hook de Render y la dirección pública del API como secreto y variable del repositorio respectivamente, y verificar que ningún valor aparece en archivos versionados con `git grep` sobre las cadenas involucradas
  > Secreto `RENDER_DEPLOY_HOOK_URL` guardado con `gh secret set` a partir del `.env` local, validando antes de enviarlo que tuviera la forma de un deploy hook; no pasó por el chat. Un primer intento guardó un valor vacío por un error del script y se corrigió; una actualización posterior desconocida se reemplazó por el valor validado. Variable `API_URL=https://idrconsultingcapacitacion.onrender.com`. `git grep` y `git log -S` con los valores reales (URL y clave del hook, contraseña de Neon de `desarrollo`): ninguno está en archivos versionados ni en el historial. Una coincidencia inicial resultó un falso positivo: el `.env` de la raíz tiene una URL de `localhost` cuya «contraseña» es una palabra corriente.
